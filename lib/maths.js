...
export async function main(ns) {
	...
	switch (typeof (ns.args[0])) {
		case 'number':
			...
			ns.tprint("Executed in "+ns.tFormat(new Date().getTime() - time))
			//}
				...
				case "ulam":
					if (ns.args[2] == "spin")
						for (let i = 0; i < 1000; i++) {
							ns.tprint(await ulam(ns, ns.args[1], i))
							await ns.sleep(0)
						}
					else
...
export async function ulam(ns, size, start = 0) { return squareSpiral(ns, size, isPrime, start) }

/** @param {NS} ns **/
export async function squareSpiral(ns, size, func, start = 0) {
